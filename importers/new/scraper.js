...
const scrapeSemester = function(semesterId) {
  return rp({
    uri: `https://api.princeton.edu/registrar/course-offerings/classes/${semesterId}`,
    headers: {
    ...
    .then(obj => obj.classes.class.map(_class => _class.course_id))
    .then(systemIds => {
      const SEMESTERS = require('../helpers/hardcode/SEMESTERS.js');
      ...
      semester.courses = systemIds.map(systemId => semesterId + systemId);
...
const scrapeCourseDetail = function(courseId) {
  ...
  */
  ...
  return rp({
    uri: `https://api.princeton.edu/registrar/course-offerings/course-details?term=${semesterId}&course_id=${systemId}`,
    headers: {
      Authorization: `Bearer ${AUTH_KEY}`,
      Accept: 'application/json'
    }
    ...
    .then(res => JSON.parse(res))
    .then(obj => obj.course_details.course_detail[0])
    .then(function(courseObj) {
      logger.log('debug', 'scrapeCourseDetail: %s starting parsing', courseId);
      return parser.parseCourseOfferingsPage(courseObj, courseId);
    })
...
// ALSO HACK TO UPDATE INSTRUCTORS
const scrapeCourseUpdate = function(courseId) {
  ...
  return Course.findById(courseId)
    .then(function(course) {
      ...
      // hack: update all instructors
      const instructorPromises = thisInstructors.map(instructorId => {
        if (!isNaN(instructorId)) {
          return instructorId;
        }

        return Instructor.findOne({
          email: `${instructorId}@princeton.edu`
        }).then(instructor => {
      ...
      return Promise.all(instructorPromises).then(instructorIds => {
        course.instructors = instructorIds;
        return course;
      });
    })
    .then(function(course) {
      ...
      courseUpdate.evaluations = evaluations;
